...
//
// L1d3 iter-5 Path 3 fixture — canonical-3-iter_arg dot inside scf.for.
//
// Authored from real Triton-emitted TTGIR captured via MLIR_ENABLE_DUMP=1
// while running test_metal_backend_dot_f32_8x8.py::test_dot_f32_8x8[16-16-16]
// (K_TILES=2 → upstream loop-unroll keeps the 2-iter scf.for). The pattern
// exercises:
//   1. iter-3 `preprocessDotCvtChains`: strips the two
//      `ttg.convert_layout #blocked → #dot_op` ops feeding tt.dot's A/B.
//   2. iter-5 Step 2 matcher relax (`tryUnrollCanonical3IterArgDot`):
//      previously rejected this body because cvt-stripped Triton emission
//      still leaves 6 ops in a specific permutation; now classifies by
//      op-type instead of strict count.
//   3. iter-5 Step 2.5 `extractBK` generalization: A's K-bump emits as
//      `arith.constant dense<8> : tensor<8x8xi32>` directly (NOT
//      `tt.splat(arith.constant 8)`).
//   4. iter-5 Step 2.6 advisory B-side BK check: B's bump emits as
//      `tt.splat(arith.muli(%stride_bk, 8))` which is non-constant, so the
//      check is skipped (no equality enforced).
//   5. iter-5 `unwrapPtrToKernelArg` extension: walks through the
//      `addptr(broadcast(addptr(splat(kernelArg), row), col))` 2-stage
//      pointer chain to reach the kernel-arg, instead of bailing at the
//      broadcast and returning a tensor-typed value defined AFTER the for
//      loop (which would have caused a dominance violation when
//      `bridgePtrToMemref` inserted the unrealized_conversion_cast before
//      the for).
//
// See `.omc/plans/l1d3-iter5-matcher-and-rewriter-fix-consensus.md` and
// `.omc/specs/deep-interview-l1d3-iter5-matcher-and-rewriter-fix.md`.

#blocked = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [4, 8], warpsPerCTA = [4, 1], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:80", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @dot_canonical_3iterarg(%a_ptr: !tt.ptr<f32>, %b_ptr: !tt.ptr<f32>, %c_ptr: !tt.ptr<f32>, %stride_am: i32, %stride_bk: i32, %stride_cm: i32) {
    ...
    %loop:3 = scf.for %_ = %c0_i32 to %c2_i32 step %c1_i32 iter_args(%a_p = %a_ptrs, %b_p = %b_ptrs, %acc_iter = %acc) -> (tensor<8x8x!tt.ptr<f32>, #blocked>, tensor<8x8x!tt.ptr<f32>, #blocked>, tensor<8x8xf32, #blocked>)  : i32 {
...
//  - the cvt-strip preprocess eliminates the two #blocked→#dot_op cvts;
//  - the canonical-3-iter_arg matcher accepts the loop body (now with cvts
//    gone, the body has 6 ops: load, load, dot, addptr, addptr, yield);
//  - the unroller emits a simdgroup chain BEFORE the for (the for is then
//    erased) — N=2 iterations + an outer c-init + final store.
//
// CHECK-LABEL: metal.kernel dot_canonical_3iterarg
