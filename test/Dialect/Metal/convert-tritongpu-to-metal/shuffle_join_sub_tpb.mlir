...
// 4x2 < threads per block: the companion mapping already holds both operands
// per row, so this must NOT move any data.
// CHECK-LABEL: metal.kernel join_sub_tpb
// CHECK-NOT: metal.threadgroup_alloca
// CHECK: arith.select
// MSL-LABEL: kernel void join_sub_tpb
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "cuda:80", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @join_sub_tpb(%x_ptr: !tt.ptr<f32>, %o_ptr: !tt.ptr<f32>) attributes {noinline = false} {
    %j = arith.constant dense<1.000000e+02> : tensor<4xf32, #blocked>
    %cst = arith.constant dense<2> : tensor<4x1xi32, #blocked1>
    %a = tt.make_range {end = 4 : i32, start = 0 : i32} : tensor<4xi32, #blocked>
    %a_0 = tt.splat %x_ptr : !tt.ptr<f32> -> tensor<4x!tt.ptr<f32>, #blocked>
    %a_1 = tt.addptr %a_0, %a : tensor<4x!tt.ptr<f32>, #blocked>, tensor<4xi32, #blocked>
    %a_2 = tt.load %a_1 : tensor<4x!tt.ptr<f32>, #blocked>
    %j_3 = arith.addf %a_2, %j : tensor<4xf32, #blocked>
    %j_4 = tt.join %a_2, %j_3 : tensor<4xf32, #blocked> -> tensor<4x2xf32, #blocked2>
    %0 = ttg.convert_layout %j_4 : tensor<4x2xf32, #blocked2> -> tensor<4x2xf32, #blocked1>
    %1 = tt.make_range {end = 4 : i32, start = 0 : i32} : tensor<4xi32, #ttg.slice<{dim = 1, parent = #blocked1}>>
    %2 = tt.expand_dims %1 {axis = 1 : i32} : tensor<4xi32, #ttg.slice<{dim = 1, parent = #blocked1}>> -> tensor<4x1xi32, #blocked1>
    %3 = arith.muli %2, %cst : tensor<4x1xi32, #blocked1>
    %4 = tt.splat %o_ptr : !tt.ptr<f32> -> tensor<4x1x!tt.ptr<f32>, #blocked1>
    %5 = tt.addptr %4, %3 : tensor<4x1x!tt.ptr<f32>, #blocked1>, tensor<4x1xi32, #blocked1>
    ...
    %8 = tt.broadcast %5 : tensor<4x1x!tt.ptr<f32>, #blocked1> -> tensor<4x2x!tt.ptr<f32>, #blocked1>
    %9 = tt.broadcast %7 : tensor<1x2xi32, #blocked1> -> tensor<4x2xi32, #blocked1>
    %10 = tt.addptr %8, %9 : tensor<4x2x!tt.ptr<f32>, #blocked1>, tensor<4x2xi32, #blocked1>
    tt.store %10, %0 : tensor<4x2x!tt.ptr<f32>, #blocked1>
    tt.return
  }
}
