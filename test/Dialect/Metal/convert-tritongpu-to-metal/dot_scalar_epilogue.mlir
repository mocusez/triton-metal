...
//
// Scalar-GEMM correctness fallback (forced via TRITON_METAL_SCALAR_DOT; the
// default path for this shape is SIMD-group — see dot_simdgroup_epilogue.mlir).
// A standalone (iters==1) f32 tt.dot whose result feeds a general
// `alpha*out + beta*C` epilogue (masked store) matches no SIMD-group matmul
// matcher, and its `#blocked -> #dot_op` operand cvts are out of the
// convert_layout legality envelope. `preprocessScalarDots` rewrites it to
// `metal.scalar_dot` before the legality walk; `ScalarDotLowering` emits a
// per-thread K-reduction (get_element A/B -> extf -> fma) inside the tile loop,
// and the alpha/beta epilogue lowers via the ordinary scalar path.
//
// CHECK-LABEL: metal.kernel
// The per-thread K reduction: scf.for carrying an f32 accumulator.
