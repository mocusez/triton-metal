...
// matmul pointer shape, with a `tt.store` that carries the canonical
// `(offs_m < M) & (offs_n < N)` mask.
//
...
//
// Plan: `.omc/plans/ac2-mn-masked-store.md`.
//
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:80", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @dot_partial_tile_masked(
    ...
    // mask = (offs_m[:, None] < M) & (offs_n[None, :] < N), matching the
    // 2D-cmpi-then-broadcast shape Triton actually emits.
    %offs_m_2d = tt.expand_dims %offs_m_full {axis = 1 : i32} : tensor<8xi32, #ttg.slice<{dim = 1, parent = #blocked}>> -> tensor<8x1xi32, #blocked>
    ...
    %offs_n_2d = tt.expand_dims %offs_n_full {axis = 0 : i32} : tensor<8xi32, #ttg.slice<{dim = 0, parent = #blocked}>> -> tensor<1x8xi32, #blocked>
