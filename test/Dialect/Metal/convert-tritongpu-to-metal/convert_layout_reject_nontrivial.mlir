...
// post-coalesce shape). The cvt first feeds elementwise arith and only then a
// store, so the store-side normalizer must not rewrite it; vectorized cvt
// staging remains L1d3 territory.
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:80", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @cvt_reject_size_per_thread_gt_one(%x_ptr: !tt.ptr<f32>, %y_ptr: !tt.ptr<f32>) {
