...
def test_vector_add():
    ...
    print(f"PASS [{DEVICE}] tutorial vector add ({size} elements)")
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel='GB/s (kernel-only on Metal)',  # Label name for the y-axis.
...
# Metal: headless save_path (no GUI display). Pre-create the directory because
# Mark._run calls plt.savefig(...) before its own os.makedirs in the finally
# block (python/triton/testing.py:562 vs :586).
if __name__ == "__main__":
    test_vector_add()
    if "--benchmark" in sys.argv:
        SAVE_PATH = "/tmp/triton-metal-bench/"
