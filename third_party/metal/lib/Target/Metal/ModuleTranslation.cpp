...
// (observed compiling python/test/unit/fixtures/metal_leet/medium-monte_carlo_integration.py through
...
void ModuleTranslation::translate(mlir::scf::ForOp op) {
  ...
  // the implementation notes AC1/AC2.
...
void ModuleTranslation::translate(mlir::Region &region) {
  _output << "{";
  {
    ...
    // `python/test/unit/fixtures/metal_leet/medium-parallel_reverse_scan_gae.py` is the minimal case —
...
void ModuleTranslation::translate(mlir::triton::metal::BinaryExpOp op) {
  ...
  // the implementation notes). All other ops
