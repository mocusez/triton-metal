...
void ModuleTranslation::translate(mlir::triton::metal::FusedAttentionOp op) {
  // ONE body. There used to be a second, `emitFusedAttentionMma_`, running both
  // matmuls on the simdgroup matrix unit and selected when the working set fit
  // Apple's 32 KiB. It was deleted 2026-08-12 because it was never faster:
  // interleaved A/B over N=1024..2048, d_head=16/32, h=2/4 and num_warps=1/2/4
  // put the two bodies within 0.5% of each other with no consistent direction.
  //
  // That is structural, not a property of this device. The threadgroup budget
  // that gated the simdgroup body also capped `bd` at 32 for `bm == bn == 32`,
  // and a 32x32 tile contracting over 32 features reads as many K/V floats as
  // it does multiplies -- the kernel is memory-bound, so matrix-unit throughput
  // buys nothing. Blocking bm/bn/bd (the follow-up that body existed to enable)
  // could not have escaped it either: a blocked tile is still budget-sized, so
  // it lands at the same arithmetic intensity.
