...
static LogicalResult rewriteReduceOperand(ConvertLayoutOp cvt,
  ...
  // The rank-1 restriction guarantees that the reduce result is a scalar, so
  // it has no layout encoding and its type does not need to change.
  return success();
}
